...
pub static COMPILERS: LazyLock<HashMap<String, CompileApp>> = LazyLock::new(|| {
    let mut m = HashMap::new();
    
    for file in COMPILERS_DIR.find("*.toml").unwrap().filter_map(|e| e.as_file()) {
        // Compile-time validations ensure only verified TOML configurations are processed. // TODO!
        let config: CompileApp = toml::from_str(file.contents_utf8().unwrap()).unwrap();
...
pub fn get_compiler(name: &str) -> &'static CompileApp {
